...
func main() {
	...
	if *versionFlag {
		fmt.Printf("GO+ Dil Sunucusu v%s\n", version)
		os.Exit(0)
	}
	...
	switch *modeFlag {
	case "stdio":
		log.Println("GO+ Dil Sunucusu başlatılıyor (stdio modu)...")
		serveStdio(ctx)
	case "tcp":
		log.Printf("GO+ Dil Sunucusu başlatılıyor (tcp modu, adres: %s)...\n", *addrFlag)
...
func (h *langHandler) Handle(ctx context.Context, conn *jsonrpc2.Conn, req *jsonrpc2.Request) {
	...
	case "textDocument/hover":
		...
		if err := conn.Reply(ctx, req.ID, map[string]interface{}{
			"contents": "GO+ dil sunucusu tarafından sağlanan bilgi",
		}); err != nil {
...
func printHelp() {
	fmt.Println("GO+ Dil Sunucusu")
	fmt.Println("\nKullanım:")
	fmt.Println("  goplsp [bayraklar]")
	fmt.Println("\nBayraklar:")
	flag.PrintDefaults()
	fmt.Println("\nÖrnekler:")
	fmt.Println("  goplsp                  # Standart giriş/çıkış üzerinden çalıştır")
	fmt.Println("  goplsp -mode=tcp        # TCP sunucu olarak çalıştır (varsayılan port: 8080)")
	fmt.Println("  goplsp -mode=tcp -addr=:9090  # TCP sunucu olarak belirtilen portta çalıştır")
	fmt.Println("  goplsp -log=goplsp.log  # Log dosyasına yaz")
}