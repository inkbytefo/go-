...
func main() {
	...
	if *versionFlag {
		fmt.Printf("GO+ Belgelendirme Aracı v%s\n", version)
		os.Exit(0)
	}
...
func startServer() {
	fmt.Printf("GO+ belgelendirme sunucusu başlatılıyor (port: %d)...\n", *portFlag)
...
func generateDocs(packages []string, format string) {
	fmt.Printf("GO+ belgelendirmesi oluşturuluyor (format: %s)...\n", format)
	...
	for _, pkg := range packages {
		...
		// Belgelendirme içeriği oluştur (gerçek implementasyonda GO+ dosyalarını ayrıştırıp belgelendirme oluşturacak)
...
func generatePackageDoc(pkg string, files []string, format string) string {
	...
	// Paket açıklaması (gerçek implementasyonda GO+ dosyalarından çıkarılacak)
...
func printHelp() {
	fmt.Println("GO+ Belgelendirme Aracı")
	fmt.Println("\nKullanım:")
	fmt.Println("  gopdoc [bayraklar] [paketler]")
	fmt.Println("\nBayraklar:")
	flag.PrintDefaults()
	fmt.Println("\nÖrnekler:")
	fmt.Println("  gopdoc                      # Mevcut dizindeki paketi belgele")
	fmt.Println("  gopdoc -html                # HTML formatında belgelendirme oluştur")
	fmt.Println("  gopdoc -markdown            # Markdown formatında belgelendirme oluştur")
	fmt.Println("  gopdoc -output=docs         # Belgelendirmeyi docs dizinine kaydet")
	fmt.Println("  gopdoc -server              # Belgelendirme sunucusu başlat")
	fmt.Println("  gopdoc -server -port=8080   # Belgelendirme sunucusunu 8080 portunda başlat")
	fmt.Println("  gopdoc ./pkg ./internal     # Belirtilen paketleri belgele")
}