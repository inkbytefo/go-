...
var (
	...
	programFlag = flag.String("program", "", "Çalıştırılacak GO-Minus programı")
)
...
func main() {
	...
	if *versionFlag {
		fmt.Printf("GO-Minus Hata Ayıklama Aracı v%s\n", version)
		os.Exit(0)
	}
	...
	switch *modeFlag {
	case "stdio":
		log.Println("GO-Minus Hata Ayıklama Aracı başlatılıyor (stdio modu)...")
		serveStdio(ctx, debugger)
	case "tcp":
		log.Printf("GO-Minus Hata Ayıklama Aracı başlatılıyor (tcp modu, adres: %s)...\n", *addrFlag)
...
// Debugger, GO-Minus programları için hata ayıklama oturumunu yönetir
...
func (d *Debugger) GetStackTrace() ([]string, error) {
	...
	return []string{"main.main (main.gom:10)", "runtime.main (runtime.gom:100)"}, nil
}
...
func printHelp() {
	fmt.Println("GO-Minus Hata Ayıklama Aracı")
	fmt.Println("\nKullanım:")
	fmt.Println("  gomdebug [bayraklar] [program]")
	fmt.Println("\nBayraklar:")
	flag.PrintDefaults()
	fmt.Println("\nÖrnekler:")
	fmt.Println("  gomdebug program.gom       # program.gom dosyasını hata ayıkla")
	fmt.Println("  gomdebug -program=program.gom  # program.gom dosyasını hata ayıkla")
	fmt.Println("  gomdebug -mode=tcp program.gom  # TCP sunucu olarak çalıştır")
	fmt.Println("  gomdebug -log=debug.log program.gom  # Log dosyasına yaz")
}